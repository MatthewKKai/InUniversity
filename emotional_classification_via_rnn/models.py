...
class Model(object):
    ...
    def recall(self):
        ...
        output = tf.nn.sigmoid(self.predict)
        # 真正类
        ok0 = tf.logical_and(tf.greater(output,0.5), tf.equal(self.label,1))
        # 假正类
        ok1 = tf.logical_and(tf.less_equal(output,0.5), tf.equal(self.label,1))
        ok = tf.logical_or(ok0, ok1)
        # 计算recall
        recall = tf.reduce_mean(tf.cast(ok,dtype=tf.float32))
        return recall
    ...
    def f1(self):
        ...
        :return:
        """
        f1 = (2*self.acc*self.recall)/(self.recall+self.acc)
        return f1
