...
def create_vocab():
    ...
    word_list = []
    # 从文件中读取数据，拆分单词
    with open(settings.NEG_TXT, 'r',errors="ignore") as f:
        f_lines = f.readlines()
        for line in f_lines:
            words = line.strip().split()
            word_list.extend(words)
    with open(settings.POS_TXT, 'r',errors="ignore") as f:
        f_lines = f.readlines()
        for line in f_lines:
            words = line.strip().split()
            word_list.extend(words)
...
def create_vec(txt_path, vec_path):
    ...
    with open(txt_path, 'r',errors="ignore") as f:
        ...
        for line in f_lines:
            ...
            vec.append(tmp_vec)
    # 写入文件中
    with open(vec_path, 'w',errors="ignore") as f:
        for tmp_vec in vec:
...
def cut_train_dev_test():
    ...
    # 使用轮盘赌法划分数据集
    with open(settings.POS_VEC, 'r',errors="ignore") as f:
        f_lines = f.readlines()
        for line in f_lines:
    ...
    with open(settings.NEG_VEC, 'r',errors="ignore") as f:
        f_lines = f.readlines()
        for line in f_lines:
            tmp_data = [int(word) for word in line.strip().split()]
            tmp_label = [0, ]
            ...
            labels[index].append(tmp_label)
    # 计算一下实际上分割出来的比例
    print( '最终分割比例', np.array([list(map(len, data))], dtype=np.float32) / sum(list(map(len, data))))
    # 打乱数据，写入到文件中
...
def decode_file(infile, outfile):
    ...
    with open(infile, 'r',encoding = "gb18030",errors="ignore") as f:
        # txt = f.read().encode("gbk",errors="ignore").decode('Windows-1252')
        txt = f.read()
